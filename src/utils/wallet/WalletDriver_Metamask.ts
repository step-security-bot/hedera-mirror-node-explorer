...
export class WalletDriver_Metamask extends WalletDriver {
    ...
    public constructor() {
        const isBraveWallet = (window as any).ethereum ? (window as any).ethereum.isBraveWallet : false;

        const wallet = isBraveWallet ? "Brave Wallet" : "Metamask"
        ...
        super(wallet, walletLogo, walletIcon)
    }
    ...
    public async connect(network: string): Promise<string[]> {
        ...
        if (provider !== null) {
            ...
            this.provider.on('chainChanged', this.handleDisconnect);
        } else {
